...
function updateSearchInputs() {
  ...
  if (category === 'currencies') {
    const searchKeys = [
      { key: 'Currency denomination', label: 'Currency denomination', type: 'text' },
      { key: 'Serial Number', label: 'Serial Number', type: 'text' }, // Text input for manual entry
      { key: 'Date', label: 'Date', type: 'text' } // Text input for manual entry
    ];
  ...
  // Handle 'rivets' category
    ...
    calcButton.addEventListener('click', () => {
      ...
      const lethalThreshold =  doseTaken  * personWeight;
  
      // Check if the dose taken is lethal
      const isLethal = lethalThreshold >= lethalDose;
  ...
  if (category === 'currencies') {
    results.forEach(result => {
      ...
      messageDiv.innerHTML += `<strong>Message:</strong> ${result.msg || 'N/A'}<br>
                                ...
                                <strong>Place:</strong> ${result.place || 'N/A'}<br>
                                <strong>stamp:</strong> ${result.stamp || 'N/A'}<br>                           
                                <strong>Degree:</strong> ${result.Degree || 'N/A'} `;
